...
# Assuming 'lib' is in the parent directory of 'api'
import sys
...
def biblio_record_to_dict(record) -> dict:
    ...
    cite_type_mapping = {
        "journal article": "article-journal", "jour": "article-journal",
        "book chapter": "chapter", "chap": "chapter",
        "book": "book", "thesis": "book", "thes": "book"
    }
...
def sru_search(user_input: str) -> list:
    ...
    # **FIXED**: Use WOE (Wörter aus allen Feldern) index for DNB general search
...
get_resolver = {
    'archive': archive_today_data, 'books.google': google_books_data,
    'encrypted.google': google_encrypted_data, 'google': google_encrypted_data,
...
def api_cite():
    ...
    try:
        ...
        template_format = params.get('template_format', 'custom')

        if not user_input:
            return jsonify("Please provide a search query."), 400
        ...
        if cache_key in rawDataCache:
            ...
            if not resolver:
                return jsonify(f"Invalid input type: {input_type}"), 400
        ...
        if isinstance(raw_data, list):
            ...
            if template_format == 'custom':
                 ...
                 outputs = [data_to_sfn_cit_ref(item, date_format='%Y-%m-%d', pipe_format=' | ', template_format=template_format) for item in raw_data]
