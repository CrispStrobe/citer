...
# --- Flask App Setup ---
# **FIXED**: Correctly define static folder path from project root
...
def biblio_record_to_dict(record) -> dict:
    ...
    return {
        'title': title, 'subtitle': subtitle, 'authors': [format_name(author) for author in (record.authors or [])],
        'editors': [format_name(editor) for editor in (record.editors or [])], 'translators': [format_name(translator) for translator in (record.translators or [])],
        'year': record.year, 'publisher': record.publisher_name, 'address': record.place_of_publication,
        'isbn': record.isbn, 'issn': record.issn, 'journal': record.journal_title, 'volume': record.volume,
        'issue': record.issue, 'page': record.pages, 'doi': record.doi, 'cite_type': cite_type,
        'series': record.series, 'edition': record.edition,
    }
...
get_resolver = {
    'archive': archive_today_data, 'books.google': google_books_data, 'encrypted.google': google_encrypted_data,
    'google': google_encrypted_data, 'jstor': jstor_data, 'ketab': ketabir_data, 'noorlib': noorlib_data,
    'noormags': noormags_data, 'web-beta.archive': archive_org_data, 'web.archive': archive_org_data,
...
input_type_to_resolver = {
    '': url_doi_isbn_data, 'url-doi-isbn': url_doi_isbn_data,
    'pmid': pmid_data, 'pmcid': pmcid_data, 'oclc': oclc_data,
    'sru': sru_search, 'ixtheo': ixtheo_search,
}
...
def api_cite():
    ...
    try:
        ...
        if cache_key in rawDataCache:
            ...
            raw_data, rawDataCache[cache_key] = resolver(user_input), raw_data
        if isinstance(raw_data, list):
            if not raw_data: return jsonify("No results found.")
            if template_format == 'custom':
                citations = [custom_format(item) for item in raw_data]
                formatted_string = "\n\n".join(citations)
            else:
                outputs = [data_to_sfn_cit_ref(item, template_format=template_format) for item in raw_data]
                format_map, idx = {'sfn': 0, 'cite': 1, 'ref': 2}, format_map.get(template_format, 1)
